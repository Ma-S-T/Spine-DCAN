...
def evaluation_inference(data_loader, model, device, num_classes, reduction='none', save_path=None):
    ...
    hd_eval = []
    with torch.no_grad():
        for step, (x, y, k) in enumerate(data_loader):#x:img(1,1,512,512)  y:label(1,512,512)  k:allmask(1,12,512,512)
            x = x.to(torch.float32).to(device)
            k = k.to(torch.float32).to(device)
            logit_map = model(x)#(1,12,512,512)
            out = torch.argmax(torch.softmax(logit_map, dim=1), dim=1)#(1,512,512)
            out_temp = torch.zeros_like(k)
            for i in range(num_classes):
                out_temp[:, i] = (out == i).to(out_temp.dtype)
            score_hd = hd95_score(out_temp[:, 1:], k[:, 1:], reduction='none')
            score_dice = dice(out_temp[:, 1:], k[:, 1:], reduction='none')
            hd_eval.append(score_hd)
            dice_eval.append(score_dice.detach().cpu().numpy())
            data_label = torch.argmax(out_temp, dim=1)[0].cpu().numpy()  # (512, 512)
            background_img = x[0][0].detach().cpu().numpy()  # (512, 512)
            if save_path:
                cmap = plt.cm.get_cmap("tab20", 12)  # 使用tab20颜色映射表，共有12种颜色
                # 初始化一个RGB图像，用于存放带颜色的分割结果
                color_label = np.zeros((data_label.shape[0], data_label.shape[1], 3))  # 创建一个全黑的RGB数组

                # 为每个类别赋予不同的颜色 (跳过背景类 0)
                for i in range(1, 12):  # 假设类别从1到11
                    color = np.array(cmap(i)[:3])  # 获取颜色映射表的RGB值
                    # 判断颜色是否接近灰色（r ≈ g ≈ b）
                    if abs(color[0] - color[1]) < 0.1 and abs(color[1] - color[2]) < 0.1:
                        color = np.array([1.0, 0.0, 0.0])  # 替换为红色 (可以换成其他颜色)
                    color_label[data_label == i] = color

                # 将灰度背景图像转换为RGB
                background_img_rgb = np.stack([background_img] * 3, axis=-1)  # 将灰度图复制为3通道 (H, W, 3)

                # 仅在标签区域显示颜色，其他地方保留背景
                combined_img = background_img_rgb / 255.0  # 将背景图像归一化到[0, 1]
                mask = data_label > 0  # 创建一个掩码，仅选择标签区域
                combined_img = background_img_rgb.copy()  # 拷贝背景图像
                combined_img[mask] = color_label[mask]  # 在掩码区域覆盖颜色

                single_dice = np.mean(score_dice.detach().cpu().numpy())
                single_hd = np.mean(score_hd)
                plt.suptitle('GT_dice_%f_hd_%f' % (single_dice, single_hd))
                plt.imshow(combined_img)

                plt.axis("off")  # 关闭坐标轴
                plt.show()
                plt.savefig(save_path + '/%d_pred整体.jpg' % step)
                plt.close()

                # **生成纯分割结果图像，背景为纯白色**
                pure_segmentation_img = np.ones((512, 512, 3))  # 创建一个全白的RGB图像
                for i in range(1, 12):  # 假设类别从1到11
                    pure_segmentation_img[data_label == i] = color_label[data_label == i]  # 将分割类别区域填充颜色
                plt.imshow(pure_segmentation_img)  # 显示纯分割结果图像
                plt.axis("off")  # 关闭坐标轴
                plt.savefig(save_path + '/%d_pred纯分割.jpg' % step)  # 保存纯分割结果图像
                plt.close()
...
def evaluation(args, model, snapshot_path, best_or_last='best', save_fig=False):
    ...
    load_model = torch.load(load_model_path, map_location="cpu",weights_only=False)
...
def robust_evaluation(args, model, snapshot_path, direction, best_or_last='best'):
    ...
    logger.info(best_or_last + '.pth')
    load_model_path = os.path.join(snapshot_path, best_or_last + '.pth')
    load_model = torch.load(load_model_path, map_location="cpu",weights_only=True)
    model.load_state_dict(load_model["model_state_dict"])
    #
    dice_test_list = []
    hd_test_list = []
    ...
    for shift_ind in range(0, d_ind[0], d_ind[1]):
        if direction <= 1:
            ...
            shift = (shift_ind, 0)
        # x:img(1,512,512)  y:label(512,512)  k:allmask(12,512,512)
        Dataset_test = aug_disk_512(args.test_dataset_path, num_classes,shift=shift, rotate=args.aug_rotate, elastic=args.aug_elastic, seed=args.seed)
        Dataset_test.set_deterministic_shift()
        testloader = DataLoader(dataset=Dataset_test, batch_size=args.batch_size_eval,num_workers=args.num_workers, pin_memory=True, shuffle=False)
        dice_test, hd_test = inference_new(testloader, model, device, num_classes)

        fig, ax = plt.subplots()
        plt.imshow(Dataset_test[0][1].squeeze(), cmap='gray')#label
        plt.imshow(Dataset_test[0][0][0], alpha=0.7)#img
        ...
        fig, ax = plt.subplots()
        plt.imshow(Dataset_test[0][2][10].squeeze(), cmap='gray')#allmask-D4
        plt.imshow(Dataset_test[0][0][0], alpha=0.7)#img
        ...
        logit_map = model(Dataset_test[0][0].unsqueeze(0).to(device))#(12,512,512)
        out = torch.argmax(torch.softmax(logit_map, dim=1), dim=1)#(12,512)
        out_temp = torch.zeros_like(Dataset_test[0][2].unsqueeze(0))#(1,12,512,512)
        for i in range(num_classes):
            out_temp[:, i] = (out == i)

        score_hd = hd95_score(out_temp[:, 1:], Dataset_test[0][2].unsqueeze(0)[:, 1:], reduction='none')
        score_dice = dice(out_temp[:, 1:], Dataset_test[0][2].unsqueeze(0)[:, 1:], reduction='none')

        fig, axes = plt.subplots(3, 4, figsize=(12, 9))  # 创建3x4的子图网格
        axes = axes.flatten()

        for i in range(num_classes):  # 12个类别
            axes[i].imshow(out_temp[0, i].detach().cpu().numpy(), cmap='gray')
            axes[i].imshow(Dataset_test[0][0][0].detach().cpu().numpy(), alpha=0.7)
            axes[i].set_title(f'Class {i}')
            axes[i].axis('off')

        single_dice = np.mean(score_dice.detach().cpu().numpy())
        single_hd = np.mean(score_hd)
        # plt.imshow(out_temp[0,3].detach().cpu().numpy(),cmap='gray')
        # plt.imshow(x[0][0].detach().cpu().numpy(),alpha=0.5)
        plt.suptitle('GT_dice_%f_hd_%f' % (single_dice, single_hd))
    ...
    ax.set_ylim(0.3, 1.0)
    ax.set_yticks(np.linspace(0.3, 1.0, 15))
    ax.grid(True)
...
def trainer(args, model, snapshot_path):
    ...
    if args.resume_training != 0:
        ...
        data = torch.load(load_model_path, map_location="cpu",weights_only=True)
    ...
    iterator = tqdm(range(epoch_start, max_epoch), ncols=100, total=max_epoch)
    loss_file_path = os.path.join(snapshot_path, 'train_loss.txt')
    dice_file_path = os.path.join(snapshot_path, 'dice_scores.txt')
    for epoch_num in iterator:
        ...
        # one epoch is done
        print(f" Skipping batch {loss_pass} due to NaN loss")
        ...
        if dice_vali > max_vali_dice:
            ...
            dice_test = None  # Or another placeholder value
        ...
        # 保存每个epoch的损失和DICE分数到文件
        #with open(loss_file_path, 'a') as f_loss:
        #    f_loss.write(f'Epoch {epoch_num + 1}: {epoch_loss}\n')
        #with open(dice_file_path, 'a') as f_dice:
        #    f_dice.write(f'Epoch {epoch_num + 1}: Train Dice: {dice_train}, Val Dice: {dice_vali}, Test Dice: {dice_test}\n')
        #绘图
        if (epoch_num + 1) % args.plot_training_epoch == 0:
            # plot and save training_process JPG
            loss_train_arr = np.array(loss_train_list)
            normalized_loss = loss_train_arr / loss_train_arr.max()
            #print(normalized_loss)
            fig, ax = plt.subplots()
            ax.plot(normalized_loss)
            ax.set_ylim(0, 1.0)
            ...
            ax[0].grid(True)
            ax[0].set_ylim(0, 1.0)
            ax[0].set_yticks(np.linspace(0, 1.0, 21))

            ax[1].plot(epoch_list, dice_test_list,'-r')

            ax[1].set_title('dice_test(red)')
            ax[1].grid(True)
            plt.show()
...
def model_selection(model_name, args):
    from model.SpineDcan import SpineDcan
    if model_name == 'SpineDCAN':
